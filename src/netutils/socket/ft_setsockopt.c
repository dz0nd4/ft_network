...
int ft_setsockopt_ttl(int *fd, int ttl, int verbose) {
  ...
  if (verbose) fprintf(stderr, "socket: Set TTL option failed.\n");
...
int ft_setsockopt_sndtimeo(int *fd, int timeout, int verbose) {
  ...
  if (setsockopt(*fd, FT_SOL_SOCKET, SO_SNDTIMEO, (const char *)&tv,
                 sizeof(tv)) == 0)
    return (FT_EXOK);

  if (verbose) fprintf(stderr, "socket: Set SNDTIMEO option failed.\n");
...
int ft_setsockopt_rcvtimeo(int *fd, int timeout, int verbose) {
  ...
  if (setsockopt(*fd, FT_SOL_SOCKET, SO_RCVTIMEO, (const char *)&tv,
                 sizeof(tv)) == 0)
    return (FT_EXOK);

  if (verbose) fprintf(stderr, "socket: Set RCVTIMEO option failed.\n");
