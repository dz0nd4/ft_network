...
/*   Updated: 2021/05/18 23:38:53 by dzonda           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int					get_next_line(int fd, char **line)
{
	...
	buff = ft_strnew(GNL_BUFF_SIZE);
	if (!line || GNL_BUFF_SIZE < 1 || fd < 0 || (read(fd, buff, 0)) < 0)
		return (-1);
	if (!(ft_gnl_init(fd, &list, &stack)))
		return (0);
	while ((stack->index = read(fd, buff, GNL_BUFF_SIZE)) > 0)
	{
